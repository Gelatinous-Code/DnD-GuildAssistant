# M1–M3 operations guide

This guide is the administrator and maintainer runbook for the weekly-event MVP. D1 is the source of truth; Discord messages are projections of that state. Never repair state by editing a bot message or by asking members to click controls in a magic sequence.

## Service ownership and dependencies

Keep the GitHub organization, Discord application, and Cloudflare account under guild/organization control with at least two maintainers. Do not make a personal machine part of normal operation. The Cloudflare Worker, D1 database, and Cron Trigger are the runtime; Raid Helper is optional and has no credentials or runtime connection to Guild Assistant.
...
| Attach Files | Optional future Guild Assistant CSV export, not M1–M3 |
...
Only a member with Manage Guild/Administrator or the configured admin authority may run setup, lifecycle mutations, publication, role repair, or reminder configuration. Sensitive responses are ephemeral. Member signup/table controls remain available to ordinary members in the configured channels.
...
| Command | Purpose |
| --- | --- |
| `/guild setup` | Set channels, roles, IANA time zone, weekly schedule, table sizes, and automation switches. |
...
| `/week retry` | Retry one failed `publish`, `open`, `lock`, `remind`, or `roles` step using its stable operation key. |
| `/week skip` | Confirm an audited skip of one `open`, `lock`, `remind`, or `archive` occurrence. |
...
3. Run `/guild setup` with scheduling and role sync paused. Use an IANA time zone such as `America/Denver`, not a fixed UTC offset, so daylight-saving changes are handled.
...
5. Configure reminders. Confirm the ephemeral preview contains the intended channel and only explicitly configured role mentions. Templates containing `@everyone`, `@here`, or an unapproved dynamic mention must be rejected.
6. Run `/roles sync dry_run: true`. Verify that the bot role is above the managed role and that only expected Guild Assistant leases would change.
7. Run one complete synthetic week from open through archive. `test/fixtures/sanitized-week.json` is safe representative data; never copy production member lists into an issue or test.
8. Re-run `/guild setup` with `scheduling_enabled: true` and, if desired,
   `role_sync_enabled: true` only after the synthetic cycle and duplicate/retry
   check succeed.

## Normal weekly lifecycle

The default schedule opens signups seven days before play and locks them 24 hours before play. The scheduler evaluates the configured local time and persists idempotency records before performing a transition.

| Phase | Normal action | Admin check |
| --- | --- | --- |
| `draft` | Scheduler or `/week open` prepares the event. | Correct title, start time, channels, and local time zone. |
| `open` | Members select GM/player or withdraw using Guild Assistant controls. | `/week status` counts; reminder audience count, never a public non-responder list. |
| `locked` | Scheduler or `/week lock` freezes planning input. The scheduler also attempts draft generation. | Review late changes/cancellations before publication. |
| `planned` | The scheduler or `/week plan` persists a revision using GM priority and 4–6 seat balancing. | Selected/unselected rationale, underfilled tables, waitlist, overrides. |
| `published` | `/week publish` posts the confirmed revision. | Message IDs recorded; table state agrees with D1; role dry run is expected. |
| `published` | Players select/change/withdraw from tables; full tables use deterministic waitlists. | Capacity risk, promotions, reminder and role-delivery status. |
| `archived` | `/week archive` makes the completed week operational history. | Final status is internally consistent; existing Raid Helper CSV may be retained for the manual Sheet process. |

Only valid forward transitions are accepted. A command delivered twice is a retry, not a request for a second event, plan, message, reminder, or role mutation. Corrections after lock require an explicit admin action and draft regeneration. A published revision is superseded rather than silently rewritten.
...
| Scheduled open or lock was missed | Cron disabled/delayed or configuration incomplete | Fix Cron/configuration, inspect `/week status`, then `/week retry` for `open` or `lock`. Do not create a second event manually. |
...
| Member cancels after publication | Published plan no longer matches availability | Record the withdrawal, regenerate/supersede through the authorized planning flow, review waitlist promotion, then dry-run roles. Historical selection follows the GM policy. |
...
| Raid Helper or its CSV is unavailable | Independent vendor/service issue | Continue the native Guild Assistant week. Delay only the legacy manual Sheet handoff. No Guild Assistant retry depends on Raid Helper. |

### Recovery sequence for partial operations

1. Stop automatic repetition if it could notify people: disable the affected reminder or scheduling switch.
...
Guild configuration remains while the bot is installed and for a documented recovery window after removal. Raid Helper CSV files and Google Sheets are controlled by guild admins outside this database; minimize local downloads and remove them after the Sheet handoff.

M1–M3 defines these limits but does not expose a self-service purge/deletion command. Until automated retention lands, the service maintainer must review retention at least quarterly and use a tested, tenant-scoped procedure. Do not run ad hoc broad SQL against production.
...
3. offer a data export when available and state what backups contain;
...
- Confirm the successor can disable scheduling and return to Raid Helper using the ADR rollback path without assuming personal hosting costs.
